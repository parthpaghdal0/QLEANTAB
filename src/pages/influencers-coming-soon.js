...
export const query = graphql`
  fragment LangComingSoonSecond on STRAPI_COMING_SOON_SECOND {
    heading {
      data {
        heading
      }
  ...
  fragment LangLayoutSecond on STRAPI_LAYOUT {
    header {
      main_logo {
        localFile {
          childImageSharp {
  ...
  fragment LangHelperTextSecond on STRAPI_HELPER_TEXT {
    NamePlaceholder
    FindButton
    EmailPlaceholder
  }
  ...
  query {
    en: strapiComingSoonSecond(locale: { eq: "en" }) {
      ...LangComingSoonSecond
    }
    sv: strapiComingSoonSecond(locale: { eq: "sv-SE" }) {
      ...LangComingSoonSecond
    }
...
export default function Home({ data }) {
  ...
  const iconsArr =
  data[GLOBAL_LANG]?.social_media_links.social_link.map(
      (el, index) => {
        let icon
  ...
  const footerArr = data[GLOBAL_LANG]?.footer_link.map((el, index) => {
    return (
      <Link
        key={index}
        className={Style.icon}
      ...
      <div className={Style.form_wrapper}>
        ...
        <Newsletter
          ...
          labels={{
            ...
            email: "Email",
            button: data[GLOBAL_LANG + "2"]?.FindButton,
            placeholder_name: data[GLOBAL_LANG + "2"]?.NamePlaceholder,
            placeholder_email: data[GLOBAL_LANG + "2"]?.EmailPlaceholder,
          }}
